...
{block name='page_content'}
    <script src="../../js/przelewy24.js" type="text/javascript"></script>
    ...
    {if $status == 'ok'}
                            ...
                            {if !empty($bank_name)}
                                <li>
                                    <input type="radio" class="bank-box"
                                           {if $bank_id == 145 && $pay_card_inside_shop}onclick="showPayJsPopup()"{/if} data-id="{$bank_id}"
        ...
        <form action="{$p24_url}" method="post" id="przelewy24Form" name="przelewy24Form"
            ...
            {if $payment_method_selected_id > 0}
                <button data-validation-required="{$validationRequired}" data-text-oneclick="{l s='Pay by OneClick' mod='przelewy24'}"  id="submitButton" class="btn btn-primary" {if $validationRequired == 2}data-validation-link="{$validationLink}" type="button" onclick="proceedPayment({$cartId})"{else} type="submit"{/if} >{l s='Pay' mod='przelewy24'}</button>
            {elseif isset($p24_blik_code)}
                <button type="submit" class="btn btn-primary">{l s='Pay by Blik' mod='przelewy24'}</button>
            {else}
                <button id="submitButton" data-validation-required="{$validationRequired}" data-text-oneclick="{l s='Pay by OneClick' mod='przelewy24'}" class="btn btn-primary" {if $validationRequired == 2}data-validation-link="{$validationLink}" type="button" onclick="proceedPayment({$cartId})"{else} type="submit"{/if}>{l s='Pay by Przelewy24' mod='przelewy24'}</button>
            {/if}
        </form>
